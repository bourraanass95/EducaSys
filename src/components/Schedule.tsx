...
export const Schedule = ({ activeRole, user }: ScheduleProps) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const teacherUsers = dedupeById(loadedUsers || []).filter(
        (u: any) => u.role === "Teacher",
      );
      const teacherStaff = dedupeById(loadedStaff || []).filter(
        (s: any) =>
  ...
  const handleSaveSchedule = async (e: React.FormEvent) => {
    ...
    if (isTeacherBusy) {
      alert(
        `Le professeur "${teacher}" a déjà un cours programmé sur ce créneau.`,
      );
      return;
    }
    ...
    try {
      if (editingSchedule) {
        ...
        await api.addGeneric("schedules", {
          ...scheduleForm,
  ...
  const activeSchedules =
    selectedFiliere === "" || selectedYear === ""
      ...
      : schedules.filter(
          (s) =>
            s.group === selectedFiliere && s.year?.toString() === selectedYear,
        );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 print:hidden">
        <div>
          ...
          <p className="text-gray-500">
            Planification hebdomadaire par programme/filière.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={handleDownloadSchedule}
            className="flex items-center justify-center gap-2 px-6 py-4 bg-gray-100 text-gray-700 hover:bg-gray-200 rounded-2xl font-black uppercase italic text-[10px] tracking-widest transition-all active:scale-95 w-full lg:w-auto"
          >
            ...
            <button
              onClick={() => {
                ...
                setScheduleForm({
                  ...
                  teacher: "",
                  room: "",
                  group: selectedFiliere || filieres[0]?.name || "",
                  year: selectedYear,
                });
                setIsModalOpen(true);
              }}
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm w-full print:hidden">
      ...
      {selectedFiliere ? (
                  ...
                  {hours.map((hour) => (
                    <tr key={hour}>
                      ...
                      {days.map((day) => {
                        ...
                        const isCovered = activeSchedules.find(
                          (s) =>
                            s.day === day &&
                            s.startTime &&
                            s.endTime &&
                        ...
                        const rowSpan =
                          startingSession && startingSession.endTime
                            ? Math.max(
                                1,
                                parseInt(startingSession.endTime) -
                                  parseInt(startingSession.startTime),
                              )
                            : 1;

                        const session = startingSession;
                        const theme = session
                          ? getSubjectColor(session.name)
                          : null;
        ...
        <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-[32px] p-20 flex flex-col items-center justify-center text-gray-400">
          ...
          <p className="font-black text-xl italic uppercase text-gray-500 mb-2">
            Sélectionnez une filière
          </p>
          <p className="text-sm font-medium">
            Veuillez choisir un programme pour afficher son emploi du temps.
          </p>
        </div>
      )}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl relative p-8"
            >
              ...
              <form onSubmit={handleSaveSchedule} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full mt-1 px-4 py-3 bg-gray-50 rounded-xl outline-none focus:ring-2 ring-blue-500"
                    >
                      {days.map((d) => (
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <select
                        value={scheduleForm.startTime || "08:00"}
                        onChange={(e) =>
                          setScheduleForm({
                            ...scheduleForm,
                        ...
                        className="w-full px-2 py-3 bg-gray-50 rounded-xl outline-none focus:ring-2 ring-blue-500"
                      >
                        {hours.map((h) => (
                          <option
                            key={`start-${h}`}
                      ...
                      <select
                        value={scheduleForm.endTime || "09:00"}
                        onChange={(e) =>
                          setScheduleForm({
                            ...scheduleForm,
                      ...
                      >
                        {hours.map((h) => (
                          <option
                            key={`end-${h}`}
                            value={`${(h + 1).toString().padStart(2, "0")}:00`}
                          >{`${(h + 1).toString().padStart(2, "0")}:00`}</option>
                        ))}
                      </select>
                    </div>
                  </div>
                ...
                <button
                  type="submit"
                  className="w-full py-4 mt-2 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-200"
                >

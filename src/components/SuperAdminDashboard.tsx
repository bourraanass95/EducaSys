...
export const SuperAdminDashboard = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "instances" | "requests" | "admins" | "history" | "profile"
  >("instances");
  const [historyTab, setHistoryTab] = useState<
    "payments" | "schools" | "requests"
  >("payments");
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.updateGeneric("users", user.id, updateData);
  ...
  const isSuperSuperAdmin =
    user?.isSuperAdmin || user?.email === "anassbourra.1995@gmail.com";
  const [showWipeConfirm, setShowWipeConfirm] = useState(false);
  const [schoolToDelete, setSchoolToDelete] = useState<any>(null);

  // Feature settings state
  const [selectedSchoolForFeatures, setSelectedSchoolForFeatures] =
    useState<any>(null);
    ...
    try {
      ...
      setAdmins(
        dedupeById(
          (usersData || []).filter(
            (u: any) =>
              u.isSuperAdmin && u.email !== "anassbourra.1995@gmail.com",
          ),
        ),
      );
  ...
  const handleSubmitSchool = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSchool) {
        ...
        const collection = editingSchool.deletedAt
          ? "deleted_schools"
          : "schools";
        const planObj = Object.values(PLANS).find(
          (p) => p.id === schoolForm.plan,
        );
        const enabled =
          schoolForm.plan === "custom"
            ? schoolForm.selectedFeatures
            : planObj
              ? planObj.enabledFeatures
              : ALL_FEATURES;
  ...
  const confirmWipeDatabase = async () => {
    ...
    try {
      ...
      if (data.success) {
        showToast(
          "Base de données réinitialisée. Rechargez la page.",
          "success",
        );
  ...
  const confirmDeleteSchool = async () => {
    ...
    if (!isSuperSuperAdmin) {
      showToast(
        "Seul le Super Admin principal peut supprimer des établissements.",
        "error",
      );
      return;
    }
  ...
  const openAddModal = (initialData?: {
    ...
    setEditingSchool(null);
    const domain = initialData?.name
      ? initialData.name.toLowerCase().replace(/[^a-z0-9]/g, "")
      : "";
    ...
    const prefix =
      `${fName.toLowerCase()}.${lName.toLowerCase()}`.replace(
        /[^a-z0-9.]/g,
        "",
      ) || "admin";
  ...
  const handleDeleteAdmin = async (id: string) => {
    if (!isSuperSuperAdmin) {
      showToast(
        "Seul un Administrateur Global peut gérer les autres admins.",
        "error",
      );
      return;
    }
  ...
  const handleDownloadReceipt = (payment: any) => {
    ...
    doc.text(
      `Date de confirmation: ${new Date(payment.confirmedAt).toLocaleString()}`,
      20,
      90,
    );
  ...
  const handlePermanentDeleteSchool = async (id: string) => {
    ...
    setConfirmDialog({
      message:
        "Supprimer DÉFINITIVEMENT cette école de l'historique ? Cette action est irréversible.",
      onConfirm: async () => {
        try {
  ...
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRequest) {
        const collection = editingRequest.deletedAt
          ? "deleted_requests"
          : "license_requests";
  ...
  const openEditModal = async (school: any, isDeleted: boolean = false) => {
    ...
    try {
      ...
      setSchoolForm({
        ...
        selectedFeatures: school.disabledFeatures
          ? ALL_FEATURES.filter((f) => !school.disabledFeatures.includes(f))
          : Object.values(PLANS).find((p) => p.id === school.plan)
              ?.enabledFeatures || [],
  ...
  const filteredSchools = schools
    .filter((s) => {
      ...
      const matchesPlan =
        planFilter === "All" || (s.plan || "free") === planFilter;
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20">
          ...
          <div className="text-left">
            <div className="flex items-center gap-3 text-blue-600 mb-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em] italic">
                Infrastructure Globale EducaSys
              </span>
            </div>
            ...
            <p className="text-gray-500 text-sm font-medium">
              Gestion multi-tenant et déploiement d'instances EducaSys
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              onClick={() => {
                if (!isSuperSuperAdmin) {
                  showToast(
                    "Seul le Super Admin principal peut réinitialiser le système.",
                    "error",
                  );
                  return;
                }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 -mt-16 relative z-20">
          ...
          {[
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic leading-none mb-1 truncate">
                  {stat.label}
                </p>
        ...
        <div className="flex gap-4 mb-6 overflow-x-auto pb-2 scrollbar-none">
          ...
          >
            ...
            {requests.filter((r) => r.status === "Pending").length > 0 && (
              <span className="bg-rose-500 text-white w-5 h-5 flex items-center justify-center rounded-full text-[9px]">
          ...
          <button
            ...
            className={`px-6 py-3 rounded-2xl font-black uppercase tracking-widest italic text-xs transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === "profile" ? "bg-blue-600 text-white shadow-xl shadow-blue-500/20" : "bg-white text-gray-500 hover:bg-gray-50"}`}
          >
            Mon Profil
          </button>
        </div>
          ...
          <div className="bg-white rounded-[48px] shadow-2xl shadow-gray-200/50 border border-white overflow-hidden">
            <div className="p-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gray-50/30">
              <div>
                ...
                <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mt-1">
                  Liste des établissements déployés
                </p>
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative group">
                    <button className="p-3 bg-white border border-gray-100 group-hover:border-gray-200 rounded-2xl text-gray-400 transition-all flex items-center gap-2 shadow-sm">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest italic hidden md:block">
                        Filtrer
                      </span>
                    </button>
                    <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-100 rounded-2xl shadow-xl p-2 hidden group-hover:block z-50">
                      <label className="text-[10px] font-black uppercase text-gray-400 p-2 block">
                        Statut
                      </label>
                      {["All", "Active", "Suspended", "Deployment"].map(
                        (st) => (
                          <button
                            key={st}
                      ...
                      <label className="text-[10px] font-black uppercase text-gray-400 p-2 block mt-4 border-t border-gray-100">
                        Plan
                      </label>
                      {["All", ...Object.values(PLANS).map((p) => p.id)].map(
                        (p) => (
                          <button
                            key={p}
                          ...
                          >
                            {p === "All"
                              ? "Tous les plans"
                              : Object.values(PLANS).find((pl) => pl.id === p)
                                  ?.name}
                          </button>
                        ),
                      )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50/80">
                  <tr>
                    <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest italic">
                      Établissement
                    </th>
                    <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest italic">
                      Plan
                    ...
                    <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest italic text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                            ...
                            <div className="flex flex-col gap-2">
                              ...
                              <div className="flex gap-2">
                                <button
                                  type="button"
                                  onClick={() =>
                                    handlePaymentAction(school.id, "paid")
                                  }
              ...
              {filteredSchools.length === 0 && !loading && (
                <div className="p-32 text-center">
                  ...
                  <p className="text-gray-400 font-medium max-w-xs mx-auto mt-2">
                    Nous n'avons trouvé aucune instance correspondant à votre
                    recherche.
                  </p>
                </div>
              )}
            ...
            <div className="p-8 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest italic">
                Système Global v4.2.0 • EducaSys Cloud Infrastructure
              </p>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[10px] font-black text-gray-600 uppercase italic tracking-widest">
              ...
              {requests.filter((req) => req.status !== "Accepted").length ===
                ...
                !loading && (
                  <div className="p-32 text-center">
                    ...
                    <p className="text-gray-400 font-medium max-w-xs mx-auto mt-2">
                      Aucune demande de licence n'a été soumise.
                    </p>
                  </div>
                )}
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-[10px] font-bold uppercase mt-1">
                  Personnes autorisées à accéder au panel global
                </p>
              </div>
              <button
                      ...
                      paymentHistory
                        ...
                        .map((h) => (
                          <tr key={`pay-${h.id}`}>
                            ...
                            <td className="px-8 py-6 font-black text-gray-900 uppercase italic text-sm">
                              {h.schoolName}
                            </td>
                            <td className="px-8 py-6">
                              <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-lg text-[10px] font-black uppercase italic border border-emerald-100">
                                Paiement {h.month}
                              </span>
                            </td>
                            <td className="px-8 py-6 text-xs text-blue-600 font-bold italic">
                              {h.confirmedBy}
                            </td>
                            <td className="px-8 py-6 text-right">
                              <div className="flex items-center justify-end gap-2 text-gray-400">
                                <button
                                  ...
                                  className="p-2 hover:bg-gray-50 hover:text-emerald-600 rounded-xl transition-all"
                                ...
                                <button
                                  ...
                                  className="p-2 hover:bg-gray-50 hover:text-blue-600 rounded-xl transition-all"
                                  title="Modifier"
                                >
                                  <Edit2 className="w-4 h-4" />
                                </button>
                    ...
                    {historyTab === "schools" &&
                      deletedSchools.map((h) => (
                        <tr key={`del-sch-${h.id}`}>
                          ...
                          <td className="px-8 py-6 italic text-gray-400 text-xs">
                            Suppression définitive de l'instance ({h.subdomain})
                          </td>
                          <td className="px-8 py-6 text-xs text-rose-600 font-bold italic">
                            {h.deletedBy}
                          </td>
                          <td className="px-8 py-6 text-right">
                            <div className="flex items-center justify-end gap-2 text-gray-400">
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  handlePermanentDeleteSchool(h.id)
                                }
                                className="p-2 hover:bg-rose-50 hover:text-rose-600 rounded-xl transition-all"
                        ...
                        <tr key={`del-req-${h.id}`}>
                          ...
                          <td className="px-8 py-6 text-right">
                            <div className="flex items-center justify-end gap-2 text-gray-400">
                              <button
                                type="button"
                                onClick={() => handleRestoreRequest(h)}
                                className="p-2 hover:bg-gray-50 hover:text-emerald-600 rounded-xl transition-all"
                                title="Restaurer"
                              >
                                <RefreshCcw className="w-4 h-4" />
                              ...
                              <button
                                type="button"
                                onClick={() => openEditRequestModal(h)}
                                className="p-2 hover:bg-gray-50 hover:text-blue-600 rounded-xl transition-all"
                                title="Voir/Modifier"
                              >
                                <Edit2 className="w-4 h-4" />
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  handlePermanentDeleteRequest(h.id)
                                }
                                className="p-2 hover:bg-rose-50 hover:text-rose-600 rounded-xl transition-all"
                                title="Supprimer définitivement"
                              >
        ...
        {activeTab === "profile" && (
          <div className="bg-white rounded-[48px] shadow-2xl shadow-gray-200/50 border border-white overflow-hidden p-8">
            <div className="max-w-2xl mx-auto">
              <h3 className="text-xl font-black text-gray-900 uppercase italic tracking-tight mb-8">
                Mon Profil Super Admin
              </h3>
              <form onSubmit={handleUpdateProfile} className="space-y-6">
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-2 italic">
                    Nom complet
                  </label>
                  <input
                    required
                    type="text"
                    className="w-full px-6 py-4 bg-gray-50 rounded-2xl outline-none font-bold text-sm"
                ...
                <div>
                  ...
                  <input
                    type="email"
                    disabled
                    className="w-full px-6 py-4 bg-gray-100 rounded-2xl outline-none font-bold text-sm cursor-not-allowed text-gray-500"
                    value={user?.email || ""}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="password"
                    placeholder="••••••••"
                    className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent focus:bg-white focus:border-blue-600 rounded-2xl outline-none font-bold text-sm transition-all shadow-inner"
                ...
                <button
                  type="submit"
                  className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black uppercase italic text-sm hover:bg-blue-500 transition-all shadow-lg shadow-blue-200 mt-4"
                >
            ...
            >
              <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                <h3 className="text-xl font-black uppercase italic">
                  {editingAdmin ? "Modifier" : "Nouvel"}{" "}
                  <span className="text-blue-600">Admin Global</span>
                </h3>
                <button
                  onClick={() => {
                    setShowAdminModal(false);
            ...
            >
              <div className="bg-white p-10 text-gray-900 relative shrink-0">
                ...
                <h2 className="text-3xl font-black uppercase italic tracking-tighter relative z-10">
                  {editingSchool ? "Modifier" : "Déployer"}{" "}
                  <span className="text-blue-600">Instance</span>
                </h2>
                <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.3em] mt-2 relative z-10 italic">
                  EducaSys Orchestration Engine
                </p>
                ...
                <form onSubmit={handleSubmitSchool} className="p-10 space-y-8">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!editingSchool) {
                          const prefix =
                            `${(schoolForm.firstName || "").toLowerCase()}.${(schoolForm.lastName || "").toLowerCase()}`.replace(
                              /[^a-z0-9.]/g,
                              "",
                            ) || "admin";
                          const generatedAdminEmail = generatedSubdomain
                            ? `${prefix}@${generatedSubdomain}.edu`
                            : "";
                          setSchoolForm({
                            ...schoolForm,
                            name,
                      ...
                      onChange={(e) => {
                        ...
                        setSchoolForm({
                          ...
                          selectedFeatures:
                            plan === "custom"
                              ? []
                              : Object.values(PLANS).find((p) => p.id === plan)
                                  ?.enabledFeatures || [],
                        });
                      }}
                    >
                    ...
                    {schoolForm.plan === "custom" && (
                      <div className="grid grid-cols-1 gap-4 mt-4 p-6 bg-gray-50 rounded-2xl">
                        {[
                          {
                            id: "notes",
                            name: "Gestion des Notes & Bulletins",
                            desc: "Gestion des notes, saisie des moyennes et édition de bulletins.",
                          },
                          {
                            id: "attendance",
                            name: "Absences & Présences",
                            desc: "Gestion de l’appel par séance et statistiques d’assiduité.",
                          },
                          {
                            id: "finance",
                            name: "Finance & Facturation",
                            desc: "Suivi de facturation, paiements et recettes scolaires.",
                          },
                          {
                            id: "academic",
                            name: "Structure Scolaire",
                            desc: "Configuration des filières, classes, matières et coefficients.",
                          },
                          {
                            id: "students",
                            name: "Base Étudiants",
                            desc: "Fiches détaillées des étudiants, inscriptions et dossiers scolaires.",
                          },
                          {
                            id: "schedule",
                            name: "Emplois du temps",
                            desc: "Planification horaire des cours, gestion des salles et créneaux.",
                          },
                          {
                            id: "library",
                            name: "Bibliothèque Digitale",
                            desc: "Inventaire des livres de la bibliothèque, emprunts et e-learning.",
                          },
                          {
                            id: "teachers",
                            name: "Gestion des Enseignants",
                            desc: "Profils des professeurs, matières assignées et attributions.",
                          },
                          {
                            id: "staff",
                            name: "Gestion du Personnel",
                            desc: "Rôles administratifs et gestion des collaborateurs de l’école.",
                          },
                          {
                            id: "director-bi",
                            name: "Analyses BI",
                            desc: "Statistiques consolidées de l'école et analyses de performances.",
                          },
                          {
                            id: "internships",
                            name: "Gestion de Stages",
                            desc: "Suivi des offres, tuteurs, soutenances et stages professionnels.",
                          },
                        ].map((feature) => (
                          <label
                            key={feature.id}
                            className="flex gap-3 bg-white p-4 rounded-xl shadow-sm cursor-pointer hover:bg-blue-50 transition-all border border-transparent hover:border-blue-200"
                          >
                            <input
                              type="checkbox"
                              className="mt-1 h-4 w-4 text-blue-600 rounded"
                              checked={schoolForm.selectedFeatures.includes(
                                feature.id,
                              )}
                              onChange={(e) => {
                                const features = e.target.checked
                                  ? [...schoolForm.selectedFeatures, feature.id]
                                  : schoolForm.selectedFeatures.filter(
                                      (f) => f !== feature.id,
                                    );
                                setSchoolForm({
                                  ...schoolForm,
                  ...
                  <div className="p-6 bg-gray-50 rounded-[32px] border border-gray-100 space-y-6">
                    <div className="col-span-2">
                      <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest italic leading-none">
                  ...
                  <div className="grid grid-cols-2 gap-6 p-6 bg-blue-50/50 rounded-[32px] border border-blue-100">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest italic ml-4 leading-none">
                        {editingSchool
                          ? "Changer Mot de Passe (laisser vide si inchangé)"
                          : "Mot de Passe Admin"}
                      </label>
                      <input
                        ...
                        type="password"
                        placeholder={
                          editingSchool ? "Nouveau mot de passe" : "••••••••"
                        }
                        className="w-full px-6 py-4 bg-white border-2 border-transparent focus:border-blue-600 rounded-[24px] outline-none font-bold text-sm transition-all shadow-inner"
                  ...
                  <div className="flex gap-4 pt-6">
                    ...
                    >
                      {editingSchool ? "Mettre à jour" : "Lancer Déploiement"}{" "}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="p-10 space-y-6">
                ...
                <div>
                  <h3 className="text-2xl font-black uppercase italic tracking-tighter text-gray-900 mb-2">
                  ...
                  <p className="text-gray-500 text-sm font-medium">
                    ...
                    . Toutes les données associées (utilisateurs, paiements)
                    seront effacées.
                  </p>
                </div>
                <div className="flex gap-4 pt-4">
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="p-10 space-y-6 bg-rose-50/30">
                ...
                <div>
                  <h3 className="text-2xl font-black uppercase italic tracking-tighter text-rose-600 mb-2">
                    Danger : Réinitialisation
                  </h3>
                  <p className="text-gray-600 text-sm font-medium">
                    Vous êtes sur le point d'effacer <strong>TOUTES</strong> les
                    écoles et données. Cette action est irréversible.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm font-medium">
                    ...
                    <span className="font-black italic text-gray-900">
                      {showPaymentReminder.name}
                    </span>{" "}
                    n'a pas encore confirmé son règlement mensuel.
                    <br />
                    <span className="text-amber-600 font-bold uppercase text-[10px] mt-2 block tracking-widest tracking-tighter italic">
                      Relance automatique après 3 jours
                    </span>
                  </p>
                </div>
                <div className="flex flex-col gap-3 pt-4">
                  <button
                    onClick={() =>
                      handlePaymentAction(showPaymentReminder.id, "paid")
                    }
        ...
        {showPaymentEditModal && editingPayment && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[120] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                <h3 className="text-xl font-black uppercase italic">
                  Modifier <span className="text-blue-600">Paiement</span>
                </h3>
                <button
                  onClick={() => {
                    setShowPaymentEditModal(false);
        ...
        {showRequestEditModal && editingRequest && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[120] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                <h3 className="text-xl font-black uppercase italic">
                  Modifier <span className="text-blue-600">Demande</span>
                </h3>
                <button
                  onClick={() => {
                    setShowRequestEditModal(false);
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[120] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-black uppercase italic">
                    Gérer les <span className="text-amber-500">Fonctions</span>
                  </h3>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-1 italic">
              ...
              <div className="p-8 space-y-6">
                <p className="text-xs text-gray-500 font-medium leading-relaxed">
                  Sélectionnez les services que vous souhaitez activer ou
                  désactiver pour cet établissement. Les fonctionnalités
                  désactivées seront masquées du tableau de bord de l'école
                  ainsi que de son menu de navigation.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar">
                  {[
                    ...
                    <div
                      key={feature.id}
                      className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:bg-gray-100/50 transition-colors"
                    >
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          disabledFeatures.includes(feature.id)
      ...
      <AnimatePresence>
        {confirmDialog && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[200] flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white rounded-[32px] shadow-2xl p-8 max-w-sm w-full text-center"
            >
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-4 bg-gray-100 rounded-2xl font-black uppercase italic text-xs hover:bg-gray-200"
                >
                  Annuler
                </button>
                <button
                  ...
                  className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase italic text-xs hover:bg-blue-700 shadow-xl shadow-blue-200"
                >
                  Confirmer
                </button>
              </div>
        ...
        {toast && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className={`fixed bottom-8 right-8 px-6 py-4 rounded-2xl shadow-2xl z-[200] font-black uppercase italic text-xs text-white ${toast.type === "error" ? "bg-rose-500 shadow-rose-200" : "bg-emerald-500 shadow-emerald-200"}`}
          >
            {toast.message}
          </motion.div>
        )}
